...
import { Divider, IconButton, RadioGroup, Table, TableBody, TableCell, TableRow,Tooltip  } from '@mui/material'
...
function ItemCell(props) {
    const { dataSet, keyShow, bgColor, textColor, endDate, _class, master, part = '' } = props;
    ...
    return dataSet.map((val, index) => {
        ...
        return <>
            ...
            <TableCell key={index} className={`col-data relative text-[1rem] text-gray-300 p-0 w-[75px] h-[40px] ${isHoliday(val.date)} ${styleTd(val.date)} ${(val[keyShow] > 0 && (styleTd(val.date) == 'fix' || styleTd(val.date) == 'do' || styleTd(val.date) == 'today')) && bgColor}`} align='center'>
                {
                    val[keyShow] > 0 ? <div>
                        <h2 className={`${keyShow == 'doPlan' ? 'doLineHorizontal ' : ''}`}> <NumericFormat displayType='text' className={`${textColor} ${keyShow == 'doPlan' ? 'doVal ' : ''} ${_class}`} allowLeadingZeros thousandSeparator="," value={val[keyShow]} decimalScale={2} /></h2>
                    </div> : (val[keyShow] < 0 ? <NumericFormat displayType='text' className='text-red-500' thousandSeparator="," value={val[keyShow]} decimalScale={2} /> : (keyShow == 'doPlan' ? <div>{delivery ? <h2 className={`${keyShow == 'doPlan' ? 'doLineHorizontal ' : ''}`}></h2> : <Tooltip title="Supplier ไม่ส่งของ">
                        <IconButton>
